# CHANGELOG

本文件记录项目已发布版本的重要更新，按版本倒序排列。

更早的中文发布说明可参考 `docs/releases/` 目录。

## v1.1.1 - 设置页文案热修复

### 本版重点
- 修复设置对话框新增“跨设备同步”区域后，部分中文文案显示为 `??` 的问题
- 恢复设置页标题、字段标签、元数据源名称与提示文本的正常中文显示
- 增加 Qt 回归测试，校验设置页关键中文文案不再退化

## v1.1.0 - 跨设备同步工作区

### 跨设备同步工作区
- 新增“跨设备同步友好模式”，可将整个工作区迁移到百度网盘等同步目录
- 工作区内统一同步 `library_registry.json`、各文库的 `settings.json`、`library.sqlite3` 与 `library_files`
- 文献库目录优先按工作区相对路径保存，降低不同设备盘符差异带来的路径失效
- 每台设备首次指向同步工作区后会在本机记录定位信息，后续启动可自动回到该工作区
- 同步模式下禁用 `link` 并默认回退到 `copy`，避免只保存本机绝对路径
- SQLite 增加 `busy_timeout` 与关闭前 checkpoint，提升同步盘环境下的稳定性

## v1.0.0 - OCR 移除与版本稳定化

### 本版重点
- 完整移除 OCR / Umi-OCR 相关能力，包括设置项、工具箱入口、后台服务与 PDF OCR 回退流程
- 保留 PDF 文本提取、全文检索、批量导出、重复检测、统计报表与维护工具等核心能力
- 清理 README 与当前版本说明，避免界面和文档继续出现 OCR 相关信息

### 兼容性说明
- 旧版 `settings.json` 中遗留的 OCR 字段会在读取时自动忽略，不影响现有文库继续使用
- 本次发布不涉及数据库结构升级，已有文献库与附件目录可直接沿用

## v0.9.0 - 附件修复、短标题与文库自定义路径

### Bug 修复
- 修复附件添加因 Windows 路径过长（MAX_PATH 260 字符限制）导致静默失败：附件现在统一存放在文库根目录，不再按文献创建子文件夹
- 修复附件添加全部失败时仍显示"成功"：根据实际结果区分 success / warning 提示，全部失败时显示具体错误原因
- 附件处理循环异常捕获从 `OSError` 扩展为 `Exception`，避免非文件系统异常中断整个批量操作

### 新功能
- 新增"短标题"字段：填写后 PDF 批量重命名将优先使用短标题生成文件名，避免长标题导致路径超限
- 新增"译题"可选列：文献列表支持显示译题列，需在列设置中手动启用
- 文库管理支持自定义文库目录：新建文库时可选择自定义路径（留空则使用默认路径）
- 文献列表新增"短标题"可选列

## v0.8.0 - 元数据合并修复与文库管理增强

### Bug 修复
- 修复抓取元数据后合并功能：确认合并后获取到的元数据无法正确写入文献记录，原因是 `_refresh_table` 中的 flush 用旧表单值覆盖了刚合并的数据；现在在读取详情和应用合并前各增加一次 flush
- 修复附件"复制到库"后静默失败：`_add_attachments` 仅捕获 `ValueError`，未覆盖文件操作可能抛出的 `OSError` / `PermissionError` / `FileNotFoundError`；现在改为 `except Exception` 并对单个文件失败做跳过处理
- `compute_checksum` 在文件不存在时返回空字符串，避免异常

### 文库管理增强
- 新增"删除文库"功能，支持仅删除注册记录或同时删除文库目录和数据库文件
- 文库管理对话框新增"文献数"列，直观展示各文库文献数量
- 隐藏"数据库"列（用户不需要看数据库路径），调整列宽
- 当前活动文库行蓝色高亮，已归档行灰色
- 双击文库行即可快速切换
- 切换文库前自动保存未保存的编辑

### 备份恢复改进
- 恢复备份前自动创建 `auto_backup_before_restore.zip`，防止误操作导致数据丢失

## v0.7.2 - 打开所在文件夹与工具箱 UX 优化

- 附件区"定位文件"改为"打开所在文件夹"，直接打开文件所在目录，不再依赖 `explorer /select,`
- 未选择文献时的 toast 提示更明确："请先在文献列表中选择一条或多条记录。"
- 全文检索：空查询拦截 + 零结果 / 有结果状态反馈
- 复制 GB/T：生成失败时提示检查标题、作者等必填字段
- PDF 重命名预览：无需修改项标注"文件名已符合规则"
- 维护工具-路径修复：修复完成后区分"全部恢复"与"仍有未解决项"
- 重复检测：零结果 toast 停留时长加长
- 导入中心：扫描前显示"正在扫描，请稍候..."，扫描后区分有结果 / 无结果

## v0.7.1 - 资源管理器定位修复与附件交互优化

- 修复"定位文件"打开错误位置：`reveal_path` 改用字符串格式调用 `explorer /select,`，解决路径含空格时 Python `list2cmdline` 引号包裹导致 Explorer 无法识别 `/select,` 标志的问题
- 修复 `resolve_path` 路径对称性：拼接时统一使用 `root.resolve()`，与 `_store_path` 基准一致
- 未选中附件时点击"打开"或"定位文件"改为 toast 提示，不再静默失败
- 附件选中状态与按钮启禁联动，取消选中后禁用操作按钮
- 新增路径含空格、中文字符的定位测试用例

## v0.7.0 - 附件定位修复与交互增强

- 修复附件"定位文件"功能失效：加载附件列表时增加守卫标志，防止 Qt 信号在清空/添加过程中错误重置当前选中 ID
- 新增 `_select_attachment_item` 辅助方法，附件选中逻辑与笔记保持一致
- 绝对路径附件也调用 `.resolve()` 统一路径解析
- 附件列表只显示文件名，悬停显示完整路径（tooltip）
- 无附件时显示空状态提示文字
- 双击附件列表项直接打开文件
- Delete 键快捷删除附件（作用域限定在附件列表）
- 删除附件后自动选中相邻项

## v0.6.2 - 附件定位文件修复

- 修复 Windows 下“附件 -> 定位文件”不能正确选中目标文件的问题
- 统一改为使用正确的 `explorer /select,<path>` 调用格式，目录和文件的打开行为分开处理
- 增加附件定位相关回归测试，覆盖文件定位、目录打开和缺失文件提示场景

## v0.6.1 - 详细信息保存策略与列表列配置升级

- 详细信息区改为“立即保存 + 自动保存”并存，新增自动保存开关与保存间隔设置
- 自动保存关闭后，编辑中会明确显示“未保存”，切换记录、刷新列表或关闭窗口前会补做一次保存，降低内容丢失风险
- 文献列表新增“列设置”，可自由选择显示哪些列、调整列顺序，并持久化记录列宽

## v0.6.0 - GB/T 7714 元数据字段补全

- 元数据模型补齐 GB/T 7714-2015 所需字段，新增副标题、编者、译者、出版地、机构、会议地点、学位类别、版次、报告号、访问日期等字段
- 元数据编辑区改为按文献类型动态显示，只保留当前类型需要填写的字段，并在切换类型时自动清理不适用内容
- 抓取、预览、合并、BibTeX / RIS 解析、GB/T / CSL 导出全部兼容上述字段扩展，抓取后仍可正确落入对应类型字段

## v0.5.1 - 更新提示文案与时间显示优化

- 检查更新走备用通道时，不再展示 HTTP 403 等技术细节，统一改为更易理解的中文提示
- Release 发布时间改为按本机时区显示，本地查看时可直接看到形如 `2026-03-15 15:54:49` 的时间
- 保留备用通道回退能力，受限网络下仍可继续完成版本检查

## v0.5.0 - 后台任务稳定性与发布整理

- 主窗口关闭时会主动取消后台任务、等待线程池收尾，并补做未落盘的元数据保存
- 同名后台任务增加去重保护，取消后的任务结果不再回写 UI，减少重复点击和退出时的异常提示
- README 重写为面向发布的中文说明，补齐安装、打包、测试与 Release 流程文档

## v0.4.0 - 元数据源有序回退

- 设置页恢复元数据源多选，可自由勾选并排列回退优先级
- 元数据抓取按配置顺序逐一尝试，命中即停
- 保留来源顺序保存逻辑，重开设置后不丢失优先级配置
- 延续 v0.3.5 的安装包启动修复和 Pillow 兼容性改进

## v0.3.5 - 启动稳定性热修复

- 修复 PIL 模块不完整导致 `module 'PIL' has no attribute '__version__'` 启动崩溃
- 打包规格文件显式收集完整的 Pillow / PIL 模块
- 运行时依赖补充 Pillow

## v0.3.4 - 开发体验与数据库优化

- 新增 `dev` extra，一次安装开发、测试和打包依赖
- SQLite 启用 WAL，增加 year / subject / reading_status 索引
- ViewModel 新增元数据保存等封装，降低 View 对 Controller 的耦合
- 元数据页增加脏检查，未改动时不触发写库

## v0.3.3 - 更新检查稳定性

- 检查更新支持 API 403 / 429 时自动回退到 Release 网页解析
- 支持 GITHUB_TOKEN / GH_TOKEN 提升受限网络成功率

## v0.3.2 - 列表刷新与状态恢复

- 文献列表刷新（按钮 + F5 快捷键）
- 后台任务状态守护，异常残留自动恢复
- 元数据合并优化：替换占位标题、作者去重、关键词去重
- 引用键支持手动编辑并自动保存
